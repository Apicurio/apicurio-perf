...
do
  CLUSTER_ID=`ocm get "$OCM_API/api/clusters_mgmt/v1/clusters" | jq -c "(.items[] | select(.name | contains(\"$CLUSTER_NAME\"))).id"`
...
do
  CLUSTER_STATUS=`ocm get "$OCM_API/api/clusters_mgmt/v1/clusters" | jq -c "(.items[] | select(.id | contains(\"$CLUSTER_ID\"))).status.state"`
  echo "  Provisioning Status: $CLUSTER_STATUS"

  CLUSTER_URL=`ocm get "$OCM_API/api/clusters_mgmt/v1/clusters" | jq -c "(.items[] | select(.id | contains(\"$CLUSTER_ID\"))).api.url"`
...
CLUSTER_CONSOLE=`ocm get "$OCM_API/api/clusters_mgmt/v1/clusters" | jq -c "(.items[] | select(.id | contains(\"$CLUSTER_ID\"))).console.url"`
